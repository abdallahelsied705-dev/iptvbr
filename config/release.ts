...
export function getCommercialReleaseStatus(): ReleaseStatus {
  ...
  const devicesReady = businessTruth.devices.verification === "verified" && businessTruth.devices.production.length > 0;
  const rightsReady = businessTruth.rights.verification === "verified" && Boolean(businessTruth.rights.production?.verified);
  const companyReady = businessTruth.company.verification === "verified" && Boolean(businessTruth.company.production);
  const trialConsistent = businessTruth.trial.verification !== "verified" || Boolean(businessTruth.trial.production);
  return pricingReady && devicesReady && rightsReady && companyReady && trialConsistent ? "ready" : "blocked";
}
...
export function getReleaseChecklist() {
  return {
    ...
    simultaneousDevices: businessTruth.simultaneousDevices.verification === "verified" && typeof businessTruth.simultaneousDevices.production === "number",
